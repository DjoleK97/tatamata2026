...
$course_idG = clean($_GET['course_id']);
$chapter_idG = clean($_GET['chapter_id']);

$clips = Database::getInstance()->getAllClipsForChapterAndCourse($chapter_idG, $course_idG);
$courses = Database::getInstance()->getAllCourses();

$course = Database::getInstance()->getCourse($course_idG);
$chapter = Database::getInstance()->getChapter($chapter_idG);

if (!empty($course)) {
  $chapters = Database::getInstance()->getAllChaptersForCourse($course['id']);
}
    ...
    <div class="container-xl">
      ...
      <div class="row g-3 mb-4 align-items-center justify-content-between">
        <div class="col">
          ...
          <h1 class="app-page-title mb-0">Clips for <a href="<?php echo BASE_URL . 'admin/courses/' . $course['id'] . '/chapters'; ?>">"<?php echo $course['name']; ?>"</a>
            < <?php echo $chapter['name']; ?></h1>
        </div>
      </div>
      <!--//row-->
                                  ...
                                  <form method="POST" class="text-left">
                                    ...
                                    <div class="mb-3">
                                      ...
                                      <select id="chapter-select-edit" name="chapter_id_edit" class="form-select" aria-label="Default select example">
                                        <?php foreach ($chapters as $chapterG) : ?>
                                          <option value="<?php echo $chapterG['chapter_id'] ?? "1"; ?>"><?php echo $chapterG['chapter_name'] ?? "Error"; ?></option>
                                        <?php endforeach; ?>
                                      </select>
                                    </div>
                      ...
                      <form method="POST" class="mt-5 mb-5 text-left" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                          ...
                          <select id="chapter-select" name="chapter_id" class="form-select" aria-label="Default select example">
                            <option value="<?php echo $chapter['id'] ?? "1"; ?>"><?php echo $chapter['name'] ?? "Error"; ?></option>
                          </select>
                        </div>
